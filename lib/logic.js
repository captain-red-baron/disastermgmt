...
async function sendAidRessource(tx) {
    ...
    if (tx.asset.aidRessource == tx.aidRessource){
        ...
        console.log('Wrong aid ressource sent');
    }
...
async function createAidRequest(tx){
    // Needed registries
    const aidRequestRegitry = await getAssetRegistry(namespace + '.AidRequest'); // eslint-disable-line no-undef
    const coordCkptRegistry = await getParticipantRegistry(namespace + 'CoordCkpt')

    // Use a factory for creation a new aid request
    const factory = getFactory(); // eslint-disable-line no-undef
    const aidRequest = factory.newResource(namespace, 'AidRequest', tx.aidRessourceId);
    ...
    aidRequest.aidAmount = tx.aidAmount;

    // Get the check point related to 
    const toCkpt = await coordCkptRegistry.get(tx.coordCkpt);
    const ckptRef = factory.newRelationship(namespace, 'CoordCkpt', toCkpt.getIdentifier());
